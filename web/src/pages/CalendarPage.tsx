...
export default function CalendarPage() {
  ...
  const close = () => { setOpen(false); setEditingId(null); setDraft(initialDraft()); setParams((current) => { ["new", "type", "id"].forEach((key) => current.delete(key)); return current; }, { replace: true }); };
  ...
  const create = (event: FormEvent) => {
    ...
    const value = { title: draft.title, starts_at: new Date(draft.starts_at).toISOString(), ends_at: new Date(draft.ends_at).toISOString(), related_type: draft.related_type || null, related_id: draft.related_id || null, notes: draft.notes, remind_at: draft.remind_at ? new Date(draft.remind_at).toISOString() : null };
