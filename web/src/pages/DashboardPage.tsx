  ...
  return <div className="dashboard-page">
    ...
    {!creators.length && !brands.length ? <EmptyState title="A clear workspace, ready for your relationships" text="Add your first influencer to start building your pipeline." action={<div className="empty-actions"><Button onClick={() => navigate("/app/influencers?new=1")}>Add influencer</Button></div>} /> : <>
      ...
      <section className="dashboard-secondary-grid">
        <div className="dash-panel analytics-panel"><div className="panel-heading"><div><span>Pipeline velocity</span><h2>Value moved per month</h2></div><span className="chart-legend"><i /> Deal value</span></div><div className="area-chart"><ResponsiveContainer width="100%" height="100%"><AreaChart data={velocity} margin={{ top: 10, right: 8, left: -24, bottom: 0 }}><defs><linearGradient id="valueGradient" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="var(--accent)" stopOpacity={0.35} /><stop offset="100%" stopColor="var(--accent)" stopOpacity={0} /></linearGradient></defs><CartesianGrid stroke="var(--line)" vertical={false} /><XAxis dataKey="month" axisLine={false} tickLine={false} tick={{ fill: "var(--muted)", fontSize: 11 }} /><YAxis axisLine={false} tickLine={false} tick={{ fill: "var(--muted)", fontSize: 11 }} tickFormatter={(value) => `$${value}k`} /><Tooltip contentStyle={{ background: "var(--surface-solid)", border: "1px solid var(--line)", borderRadius: 10, fontSize: 12 }} formatter={(value) => [`$${value}k`, "Value"]} /><Area type="monotone" dataKey="value" stroke="var(--accent)" strokeWidth={2.5} fill="url(#valueGradient)" /></AreaChart></ResponsiveContainer></div><div className="analytics-foot"><span><b>{campaigns.length ? Math.round(completed.length / campaigns.length * 100) : 0}%</b> campaign completion</span><span><b>{campaigns.filter((item) => item.status === "active" && isOverdue(item.due_date)).length}</b> overdue</span><span><b>{brands.length}</b> brands in pipeline</span></div></div>
