...
import { Archive, ArrowDownAZ, ArrowLeft, ArrowRight, Columns3, Download, ExternalLink, FileSpreadsheet, Filter, LayoutGrid, List, Merge, MoreHorizontal, Plus, Send, SlidersHorizontal, Sparkles, Star, Upload, X } from "lucide-react";
...
import { CSV_BOM, compact, dateLabel, download, followupWords, isValidEmail, isValidUrl, lossReasonLabel, money, normalize, relativeTime, toCSV, today } from "../lib/utils";
import { ENTITY_STATUSES, PLATFORMS, STATUS_LABELS, type Creator, type EntityStatus, type Platform } from "../types";

type CreatorDraft = {
  name: string; contact_email: string; channel_link: string; niche: string; avg_views: string; engagement_rate: string;
  platform: Platform; pipeline_status: EntityStatus; date_contacted: string; notes: string; next_action: string;
};

const emptyDraft: CreatorDraft = { name: "", contact_email: "", channel_link: "", niche: "", avg_views: "", engagement_rate: "", platform: "YouTube", pipeline_status: "new", date_contacted: "", notes: "", next_action: "" };
...
const draftToCreator = (draft: CreatorDraft) => ({
  ...
  avg_views: Number(draft.avg_views) || 0, engagement_rate: Number(draft.engagement_rate) || 0, platform: draft.platform,
...
export default function InfluencersPage() {
  ...
  const dismissTip = () => {
    setTipHidden(true);
    try {
      localStorage.setItem("if-sponsor-tip-hidden", "1");
    } catch {
  ...
  const filtered = useMemo(() => {
    const result = data.creators.filter((item) => !item.archived_at).filter((item) => {
      ...
      return matchesQuery && matchesStatus && matchesPlatform && matchesNiche && matchesEngagement && (!minViews || item.avg_views >= Number(minViews));
    });
  ...
  }, [data.creators, query, status, platform, niche, engagement, minViews, sort]);
  ...
  const exportRows = (rows: Creator[]) => {
    download("influenceflow-influencers.csv", toCSV(rows.map((item) => ({ name: item.name, contact_email: item.contact_email, channel_link: item.channel_link, niche: item.niche, platform: item.platform, avg_views: item.avg_views, engagement_rate: item.engagement_rate, pipeline_status: item.pipeline_status, date_contacted: item.date_contacted || "", next_action: item.next_action || "", notes: item.notes || "" })), { name: "Name", contact_email: "Email", channel_link: "Channel", niche: "Niche", platform: "Platform", avg_views: "Avg views", engagement_rate: "Engagement %", pipeline_status: "Status", date_contacted: "Date contacted", next_action: "Next action", notes: "Notes" }), "text/csv");
  ...
  return <div className="entity-page">
    ...
    {!filtered.length ? <EmptyState icon="search" title={query || status !== "all" ? "No influencers match this view" : "Build your creator pipeline"} text={query || status !== "all" ? "Clear a filter or try another search." : "Add creators manually or import a clean CSV with duplicate protection."} action={<Button onClick={() => setCreateOpen(true)}><Plus size={15} /> Add influencer</Button>} /> : view === "kanban" ? <CreatorKanban creators={filtered} /> : view === "cards" ? <CreatorCards creators={filtered} /> : <CreatorTable creators={filtered} selected={selected} setSelected={setSelected} density={density} />}
...
function CreatorTable({ creators, selected, setSelected, density }: { creators: Creator[]; selected: string[]; setSelected: (ids: string[]) => void; density: string }) {
  const all = creators.length > 0 && creators.every((item) => selected.includes(item.id));
  return <><div className={`data-table-wrap density-${density}`}><table className="data-table creator-table"><thead><tr><th><input type="checkbox" checked={all} onChange={() => setSelected(all ? [] : creators.map((item) => item.id))} /></th><th>Influencer</th><th>Platform / niche</th><th>Performance</th><th>Status</th><th>Contacted</th><th>Next action</th><th /></tr></thead><tbody>{creators.map((item) => <tr key={item.id}><td><input type="checkbox" checked={selected.includes(item.id)} onChange={() => setSelected(selected.includes(item.id) ? selected.filter((id) => id !== item.id) : [...selected, item.id])} /></td><td><Link className="person-cell" to={`/app/influencers/${item.id}`}><Avatar name={item.name} /><span><strong>{item.name}</strong><small>{item.contact_email || "No email added"}</small></span></Link></td><td><span className="stacked-cell"><strong>{item.platform}</strong><small>{item.niche || "No niche"}</small></span></td><td><span className="performance-cell"><strong>{compact(item.avg_views)} <small>avg</small></strong><em>{item.engagement_rate}% engagement</em></span></td><td><StatusBadge status={item.pipeline_status} /></td><td>{dateLabel(item.date_contacted)}</td><td><span className={item.next_action ? "next-action" : "muted"}>{item.next_action || "None set"}</span></td><td><Link className="row-action" to={`/app/influencers/${item.id}`}><ArrowRight size={15} /></Link></td></tr>)}</tbody></table></div><div className="table-mobile-fallback"><CreatorCards creators={creators} /></div></>;
}
...
function ImportCreators({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const preview = (raw: string) => {
    ...
    const hasHeader = parsed[0]?.some((cell) => /name|email|niche|channel|platform|status|company|domain|date|notes/i.test(cell));
    ...
    const next = values.map((cells, index) => {
      ...
      return { ...emptyDraft, row: index + 1 + (hasHeader ? 1 : 0), name, contact_email: email, channel_link: channel, niche: nicheValue, avg_views: viewsValue, engagement_rate: engagementValue, platform: (PLATFORMS.includes(platformValue as Platform) ? platformValue : "Other") as Platform, pipeline_status: (ENTITY_STATUSES.includes(statusValue as EntityStatus) ? statusValue : "new") as EntityStatus, date_contacted: /^\d{4}-\d{2}-\d{2}$/.test(dateValue) ? dateValue : "", next_action: nextActionValue, notes: hasHeader ? column(trimmed, ["notes"]) : "", errors, duplicate };
    });
    setRows(next);
  };
  ...
  const template = () => download("influenceflow-influencer-template.csv", CSV_BOM + "Name,Email,Channel,Niche,Platform,Avg views,Engagement %,Status,Date contacted,Next action,Notes\nMaya Chen,maya@example.com,https://youtube.com/@maya,Design,YouTube,150000,8.4,new,2026-09-01,Send intro,Add context here", "text/csv");
...
export function InfluencerDetailPage() {
  ...
  return <div className="detail-page"><Link to="/app/influencers" className="back-link"><ArrowLeft size={15} /> Influencers</Link><header className="detail-header"><div className="detail-identity"><Avatar name={creator.name} size="lg" /><div><span className="eyebrow">Influencer profile</span><h1>{creator.name}</h1><div><StatusBadge status={creator.pipeline_status} /><span>{creator.platform}</span><span>{creator.niche || "No niche"}</span>{creator.pipeline_status === "denied" && creator.lost_at ? <button className="lost-inline" onClick={() => data.reviewLoss("creator", creator.id)} title="Edit loss reason"><span>Lost · {lossReasonLabel(creator.lost_reason)} · {dateLabel(creator.lost_at)}</span></button> : creator.pipeline_status === "denied" ? <span className="muted">Closed early — no active deal</span> : null}</div></div></div><div className="page-actions"><Button variant="secondary" onClick={() => setMergeOpen(true)}><Merge size={15} /> Merge</Button><Button variant="secondary" onClick={() => setEditOpen(true)}>Edit profile</Button><button className="icon-btn danger" onClick={archive}><Archive size={17} /></button></div></header>
    ...
    <div className="detail-grid"><main><section className="detail-section"><div className="detail-section-head"><div><span>Contact</span><h2>Reach {creator.name.split(" ")[0]}</h2></div></div><div className="contact-block"><div><small>Email</small><strong>{creator.contact_email || "Not added"}</strong>{creator.contact_email && <a href={`mailto:${creator.contact_email}`}>Send email <ExternalLink size={13} /></a>}</div><div><small>Channel</small><strong>{creator.channel_link || "Not added"}</strong>{creator.channel_link && <a href={creator.channel_link} target="_blank" rel="noreferrer">Open profile <ExternalLink size={13} /></a>}</div><div><small>Date contacted</small><strong>{dateLabel(creator.date_contacted)}</strong></div><div><small>Follow-ups</small><strong>{creator.followup_count > 0 ? `${followupWords(creator.followup_count)}${creator.last_followup_at ? ` · ${dateLabel(creator.last_followup_at)}` : ""}` : "None yet"}</strong></div></div></section>
      <section className="detail-section"><div className="detail-section-head"><div><span>Outreach</span><h2>Follow-up status</h2></div><span className="followup-count-pill">{creator.followup_count || 0} logged</span></div><div className="followup-block"><div><strong>{followupWords(creator.followup_count || 0)}</strong><p>Each log stamps this influencer's outreach history and activity trail — the momentum panel counts it instantly.</p></div><Button onClick={() => { data.logFollowup(creator.id); const total = (creator.followup_count || 0) + 1; toast(total === 1 ? "First follow-up logged" : `Follow-up logged (${total} total)`); }}><Send size={15} /> {(creator.followup_count || 0) === 0 ? "Log follow-up" : "Log follow-up again"}</Button></div></section>
