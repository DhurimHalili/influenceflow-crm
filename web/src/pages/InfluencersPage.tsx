...
type CreatorDraft = {
  name: string; contact_email: string; channel_link: string; niche: string; avg_views: string; engagement_rate: string; stars: number;
  platform: Platform; pipeline_status: EntityStatus; date_contacted: string; notes: string; next_action: string;
};

const emptyDraft: CreatorDraft = { name: "", contact_email: "", channel_link: "", niche: "", avg_views: "", engagement_rate: "", stars: 0, platform: "YouTube", pipeline_status: "new", date_contacted: "", notes: "", next_action: "" };
...
const draftToCreator = (draft: CreatorDraft) => ({
  ...
  avg_views: Number(draft.avg_views) || 0, engagement_rate: Number(draft.engagement_rate) || 0, stars: Math.min(5, Math.max(0, Math.round(draft.stars) || 0)), platform: draft.platform,
...
export default function InfluencersPage() {
  ...
  const filtered = useMemo(() => {
    const result = data.creators.filter((item) => !item.archived_at).filter((item) => {
      ...
      return matchesQuery && matchesStatus && matchesPlatform && matchesNiche && matchesEngagement && matchesRating && (!minViews || item.avg_views >= Number(minViews));
    });
  ...
  }, [data.creators, query, status, platform, niche, engagement, rating, minViews, sort]);
  ...
  const exportRows = (rows: Creator[]) => {
    download("influenceflow-influencers.csv", toCSV(rows.map((item) => ({ name: item.name, contact_email: item.contact_email, channel_link: item.channel_link, niche: item.niche, platform: item.platform, avg_views: item.avg_views, engagement_rate: item.engagement_rate, stars: item.stars || 0, pipeline_status: item.pipeline_status, date_contacted: item.date_contacted || "", next_action: item.next_action || "", notes: item.notes || "" })), { name: "Name", contact_email: "Email", channel_link: "Channel", niche: "Niche", platform: "Platform", avg_views: "Avg views", engagement_rate: "Engagement %", stars: "Rating", pipeline_status: "Status", date_contacted: "Date contacted", next_action: "Next action", notes: "Notes" }), "text/csv");
...
function CreatorCards({ creators }: { creators: Creator[] }) {
  return <div className="mobile-card-grid">{creators.map((item) => <Link to={`/app/influencers/${item.id}`} className="entity-mobile-card" key={item.id}><header><Avatar name={item.name} /><div><strong>{item.name}</strong><span>{item.platform} / {item.niche || "No niche"}</span><Stars value={item.stars || 0} size={11} /></div><MoreHorizontal /></header><div><span><small>Avg. views</small><b>{compact(item.avg_views)}</b></span><span><small>Engagement</small><b>{item.engagement_rate}%</b></span></div><footer><StatusBadge status={item.pipeline_status} /><span>{item.next_action || "No next action"}</span></footer></Link>)}</div>;
}
...
function ImportCreators({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const preview = (raw: string) => {
    ...
    const next = values.map((cells, index) => {
      ...
      return { ...emptyDraft, row: index + 1 + (hasHeader ? 1 : 0), name, contact_email: email, channel_link: channel, niche: nicheValue, avg_views: viewsValue, engagement_rate: engagementValue, stars: /^[0-5]$/.test(starsValue.trim()) ? Number(starsValue) : 0, platform: (PLATFORMS.includes(platformValue as Platform) ? platformValue : "Other") as Platform, pipeline_status: (ENTITY_STATUSES.includes(statusValue as EntityStatus) ? statusValue : "new") as EntityStatus, date_contacted: /^\d{4}-\d{2}-\d{2}$/.test(dateValue) ? dateValue : "", next_action: nextActionValue, notes: hasHeader ? column(trimmed, ["notes"]) : "", errors, duplicate };
    });
    setRows(next);
  };
  ...
  const template = () => download("influenceflow-influencer-template.csv", CSV_BOM + "Name,Email,Channel,Niche,Platform,Avg views,Engagement %,Rating,Status,Date contacted,Next action,Notes\nMaya Chen,maya@example.com,https://youtube.com/@maya,Design,YouTube,150000,8.4,5,new,2026-09-01,Send intro,Add context here", "text/csv");
...
function CreatorEdit({ creator, onClose }: { creator: Creator; onClose: () => void }) {
  ...
  const [draft, setDraft] = useState<CreatorDraft>({ name: creator.name, contact_email: creator.contact_email, channel_link: creator.channel_link, niche: creator.niche, avg_views: String(creator.avg_views), engagement_rate: String(creator.engagement_rate), stars: creator.stars || 0, platform: creator.platform, pipeline_status: creator.pipeline_status, date_contacted: creator.date_contacted || "", notes: creator.notes, next_action: creator.next_action || "" });
