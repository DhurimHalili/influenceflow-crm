...
import { CSV_BOM, compact, dateLabel, download, engagementStars, followupWords, isPastDay, isTodayDay, isValidEmail, isValidUrl, lossReasonLabel, money, normalize, overallStars, PRIORITY_LABELS, RATING_DIMS, relativeTime, toCSV, today } from "../lib/utils";
import { ENTITY_STATUSES, PLATFORMS, STATUS_LABELS, type Creator, type EntityStatus, type Platform, type Priority } from "../types";

type CreatorDraft = {
  name: string; contact_email: string; channel_link: string; niche: string; avg_views: string; engagement_rate: string; stars_consistency: number; stars_demographics: number; stars_niche: number; priority: Priority; next_action_date: string;
  platform: Platform; pipeline_status: EntityStatus; date_contacted: string; notes: string; next_action: string;
};

const emptyDraft: CreatorDraft = { name: "", contact_email: "", channel_link: "", niche: "", avg_views: "", engagement_rate: "", stars_consistency: 0, stars_demographics: 0, stars_niche: 0, priority: "none", next_action_date: "", platform: "YouTube", pipeline_status: "new", date_contacted: "", notes: "", next_action: "" };

const dimClamp = (v: number) => Math.min(5, Math.max(0, Math.round(v) || 0));
const draftDims = (draft: CreatorDraft) => ({ stars_consistency: dimClamp(draft.stars_consistency), stars_demographics: dimClamp(draft.stars_demographics), stars_niche: dimClamp(draft.stars_niche) });
const draftToCreator = (draft: CreatorDraft) => {
  const dims = draftDims(draft);
  return {
    name: draft.name.trim(), contact_email: draft.contact_email.trim(), channel_link: draft.channel_link.trim(), niche: draft.niche.trim(),
    avg_views: Number(draft.avg_views) || 0, engagement_rate: Number(draft.engagement_rate) || 0, ...dims, stars: overallStars(dims), priority: (["none", "soon", "urgent"] as const).includes(draft.priority) ? draft.priority : "none" as Priority, next_action_date: draft.next_action_date || null, platform: draft.platform,
...
export default function InfluencersPage() {
  ...
  const setCardStatus = (id: string, pipeline_status: EntityStatus) => {
    const target = data.creators.find((c) => c.id === id);
    data.updateCreator(id, { pipeline_status, date_contacted: pipeline_status === "contacted" && !target?.date_contacted ? today() : target?.date_contacted });
    toast(`Moved to ${STATUS_LABELS[pipeline_status]}`);
  };
  ...
  const exportRows = (rows: Creator[]) => {
    download("influenceflow-influencers.csv", toCSV(rows.map((item) => ({ name: item.name, contact_email: item.contact_email, channel_link: item.channel_link, niche: item.niche, platform: item.platform, avg_views: item.avg_views, engagement_rate: item.engagement_rate, stars: item.stars || 0, consistency: item.stars_consistency || 0, demographics: item.stars_demographics || 0, niche_fit: item.stars_niche || 0, priority: item.priority, pipeline_status: item.pipeline_status, date_contacted: item.date_contacted || "", next_action: item.next_action || "", action_due: item.next_action_date || "", notes: item.notes || "" })), { name: "Name", contact_email: "Email", channel_link: "Channel", niche: "Niche", platform: "Platform", avg_views: "Avg views", engagement_rate: "Engagement %", stars: "Rating", consistency: "Consistency", demographics: "Demographics", niche_fit: "Niche fit", priority: "Priority", pipeline_status: "Status", date_contacted: "Date contacted", next_action: "Next action", action_due: "Action due", notes: "Notes" }), "text/csv");
...
function ImportCreators({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const preview = (raw: string) => {
    ...
    const hasHeader = parsed[0]?.some((cell) => /name|email|niche|channel|platform|status|company|domain|date|notes|rating|star|consistency|demographics|alignment/i.test(cell));
    ...
    const next = values.map((cells, index) => {
      ...
      const dimValue = (names: string[]) => { const v = hasHeader ? column(trimmed, names) : ""; return /^[0-5]$/.test(v.trim()) ? Number(v) : 0; };
      const dimCon = dimValue(["consistency", "posting_consistency"]);
      const dimAud = dimValue(["demographics", "audience_demographics", "audience"]);
      const dimNiche = dimValue(["niche_fit", "niche_alignment", "alignment"]);
      const rated = /^[0-5]$/.test(starsValue.trim()) ? Number(starsValue) : 0;
      const finalDims = [dimCon, dimAud, dimNiche].every((d) => d === 0) && rated > 0 ? [rated, rated, rated] : [dimCon, dimAud, dimNiche];
  ...
  const template = () => download("influenceflow-influencer-template.csv", CSV_BOM + "Name,Email,Channel,Niche,Platform,Avg views,Engagement %,Consistency,Demographics,Niche fit,Priority,Status,Date contacted,Action due,Next action,Notes\nMaya Chen,maya@example.com,https://youtube.com/@maya,Design,YouTube,150000,8.4,5,4,5,urgent,new,2026-09-01,2026-09-05,Send intro,Add context here", "text/csv");
...
export function InfluencerDetailPage() {
  ...
  return <div className="detail-page"><Link to="/app/influencers" className="back-link"><ArrowLeft size={15} /> Influencers</Link><header className="detail-header"><div className="detail-identity"><Avatar name={creator.name} size="lg" /><div><span className="eyebrow">Influencer profile</span><h1>{creator.name}</h1><div><StatusBadge status={creator.pipeline_status} /><span>{creator.platform}</span><span>{creator.niche || "No niche"}</span><Stars value={creator.stars || 0} />{creator.pipeline_status === "denied" && creator.lost_at ? <button className="lost-inline" onClick={() => data.reviewLoss("creator", creator.id)} title="Edit loss reason"><span>Lost · {lossReasonLabel(creator.lost_reason)} · {dateLabel(creator.lost_at)}</span></button> : creator.pipeline_status === "denied" ? <span className="muted">Closed early — no active deal</span> : null}</div></div></div><div className="page-actions"><Button variant="secondary" onClick={() => setMergeOpen(true)}><Merge size={15} /> Merge</Button><Button variant="secondary" onClick={() => setEditOpen(true)}>Edit profile</Button><button className="icon-btn danger" onClick={archive}><Archive size={17} /></button></div></header>
    ...
    <div className="detail-grid"><main><section className="detail-section"><div className="detail-section-head"><div><span>Contact</span><h2>Reach {creator.name.split(" ")[0]}</h2></div></div><div className="contact-block"><div><small>Email</small><strong>{creator.contact_email || "Not added"}</strong>{creator.contact_email && <a href={`mailto:${creator.contact_email}`}>Send email <ExternalLink size={13} /></a>}</div><div><small>Channel</small><strong>{creator.channel_link || "Not added"}</strong>{creator.channel_link && <a href={creator.channel_link} target="_blank" rel="noreferrer">Open profile <ExternalLink size={13} /></a>}</div><div><small>Date contacted</small><strong>{dateLabel(creator.date_contacted)}</strong></div><div><small>Follow-ups</small><strong>{creator.followup_count > 0 ? `${followupWords(creator.followup_count)}${creator.last_followup_at ? ` · ${dateLabel(creator.last_followup_at)}` : ""}` : "None yet"}</strong></div></div></section>
      ...
      <section className="detail-section"><div className="detail-section-head"><div><span>Evaluation</span><h2>Creator ratings</h2></div><span className="overall-stars"><Stars value={overallStars(creator)} /> <b>{overallStars(creator)}/5</b></span></div><div className="rating-rows">{RATING_DIMS.map((dim) => (<div className="rating-row" key={dim.key}><div><strong>{dim.label}</strong><small>{dim.hint}</small></div><StarInput size={18} value={creator[dim.key] || 0} onChange={(v) => data.updateCreator(creator.id, { [dim.key]: v } as Partial<Creator>)} /></div>))}<div className="rating-row rating-derived"><div><strong>Engagement score</strong><small>Measured from {creator.engagement_rate}% average — not rated by you</small></div><span className="derived-stars"><Stars value={engagementStars(creator.engagement_rate)} size={18} /><b>{creator.engagement_rate}%</b></span></div></div></section>
