...
import { compact, dateLabel, download, followupWords, isValidEmail, isValidUrl, lossReasonLabel, money, normalize, relativeTime, toCSV, today } from "../lib/utils";
...
export default function InfluencersPage() {
  ...
  const exportRows = (rows: Creator[]) => {
    download("influenceflow-influencers.csv", toCSV(rows.map(({ user_id: _user, archived_at: _archived, ...item }) => ({ ...item, date_contacted: item.date_contacted || "" }))), "text/csv");
...
function ImportCreators({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const preview = (raw: string) => {
    ...
    const hasHeader = parsed[0]?.some((cell) => /name|email|niche|channel|platform/i.test(cell));
    ...
    const next = values.map((cells, index) => {
      ...
      const engagementValue = hasHeader ? column(trimmed, ["engagement_rate", "engagement", "er"]).replace("%", "") : "";
      ...
      return { ...emptyDraft, row: index + 1 + (hasHeader ? 1 : 0), name, contact_email: email, channel_link: channel, niche: nicheValue, avg_views: viewsValue, engagement_rate: engagementValue, platform: (PLATFORMS.includes(platformValue as Platform) ? platformValue : "Other") as Platform, pipeline_status: (ENTITY_STATUSES.includes(statusValue as EntityStatus) ? statusValue : "new") as EntityStatus, notes: hasHeader ? column(trimmed, ["notes"]) : "", errors, duplicate };
    });
    setRows(next);
  };
  ...
  const template = () => download("influenceflow-influencer-template.csv", "name,email,channel,niche,platform,avg_views,engagement_rate,status,notes\nMaya Chen,maya@example.com,https://youtube.com/@maya,Design,YouTube,150000,8.4,new,Add context here", "text/csv");
